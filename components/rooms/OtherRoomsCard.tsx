...
function OtherRoomsCard(props) {
  ...
  return (
    <div className="group h-full flex flex-col min-h-[520px] bg-white rounded-lg border border-gray-200 p-4  transition-shadow duration-300">
      ...
      <div className="relative w-full aspect-[4/3] overflow-hidden rounded-lg mb-6 bg-gray-100">
        {Slug ? (
          <Link href={Slug} className="block h-full w-full relative">
            <Image
      ...
      <div className="flex flex-grow flex-col items-start px-1">
        ...
        {Title && <h6 className="mb-3 text-2xl  text-primary transition-colors">{Slug ? <Link href={Slug}>{Title}</Link> : Title}</h6>}

        {/* Description: Light gray, readable text */}
        {Description && <p className="mb-6 text-sm text-secondary leading-relaxed line-clamp-3">{Description}</p>}
