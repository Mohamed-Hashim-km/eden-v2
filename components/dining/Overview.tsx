...
function Overview() {
  ...
  const getTabLabel = (key) => {
    switch (key) {
      ...
      case "oyster": return "Oyster Bay";
      case "forest": return "Food & Forest";
      default: return key;
    }
  };
      ...
      <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="flex w-full md:w-full lg:w-full border-b border-gray-200">
            {["tulip", "oyster", "forest"].map((tab) => (
              ...
              >
                ...
                {activeTab === tab && (
                  <span className="absolute bottom-[-1px] left-0 w-full h-[2px] bg-gray-800" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {overviewData[activeTab].map((item) => (
            <div 
              key={item.id} 
              className="relative h-[450px] w-full rounded-lg overflow-hidden group"
            >
              {/* Image Layer */}
              ...
              <div className="absolute top-10 left-8 right-8 z-10">
                <h3 className="text-white text-4xl font-serif tracking-wide">
                  {item.title}
                </h3>
              </div>
